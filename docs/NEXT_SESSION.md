...
5. **Phantom obstacles from repeated structures (gate, siding, gravel).** Fix built 2026-09-26 (`8a4afb2`, on `fm-speed`): a speckle filter and a depth-aware person mask in the depth corridor. Field validation in motion is still open.
   ...
   - Evidence: 45 raw frames (`logs/depth_snapshots/*.npz` on the robot, copied to the session scratchpad) from 11 setups that Kevin placed with the robot parked and disarmed. The gate phantoms are false stereo matches: 150 to 500 tiny blobs of near pixels (median 2 px, largest 95 px) at random depths from 0.35 m to 1.5 m, with nothing in front.
   - Fix 1: `ObstacleAvoidanceConfig.corridor_min_blob_px = 80`. Near pixels in 8-connected blobs smaller than 80 px leave the corridor before the support rule. New log field `obstacle.corridor_speckle_px` counts them.
   - Fix 2: `ObstacleAvoidanceConfig.person_mask_depth_margin_m = 0.5`. The corridor used to zero the whole person box, which also hid an obstacle between the robot and the person inside that outline (Kevin asked why people are masked). Now only pixels at or beyond the person's range minus 0.5 m are masked. A person with an unknown range is not masked.
   - Replay of the production code, old -> new: gate phantoms 0.70-0.81 m -> 2.2 m (ground edge); gate bars at 3 ft 1.00 -> 1.03 m; trash can 0.87 m (same); woven wire fence at 3 ft 0.79 -> 1.0 m and at 5 ft 0.65 -> 1.74 m (the fence's histogram peak); siding in shade 1.73 m (same); sunlit siding at a taped 53 in (1.346 m, stereo median 1.341 m) 0.73 -> 0.78 m. A 200 px threshold lost the wire fence in one frame.
   - Still open: (a) a drive-by validation at the gate and along the barn; (b) a smooth, low-texture real obstacle (garage door, painted wall, in dim light), because sparse valid pixels could be dropped as speckle; (c) the sunlit siding reads about 0.55 m short (a 5 percent tail of false near pixels inside the wall's own blob sets the k-th smallest value): conservative, not a hazard. Rollback: `corridor_min_blob_px = 0`, `person_mask_depth_margin_m = 0.0`.
