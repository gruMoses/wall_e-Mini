...
Updated 2026-09-26 (afternoon): follow-me top speed 1.61 m/s, follow range 8 m and a 4 ft stop are LIVE (`c3c995a`, deployed 13:45). Phantom obstacles in front of an iron gate: fix built and replay-validated on 45 raw frames (section 4, item 5).
Updated 2026-09-26 (evening): I2C bus 1 froze on 2026-09-25 21:36 (GPS and UPS monitor dark for 17 h); an auto-unlock guard service is LIVE (`2759465`, section 4, item 0). The corridor phantom fix is LIVE (`cceb2e1`, item 5).
...
0. **INCIDENT 2026-09-25: follow-me drove on a frozen detection. Deploy 1 of the fix is LIVE (`2583757`, deployed 19:20, verified on the robot: `det_fresh_age_s` 0.2 s, sequence advancing, gate wired in the control loop). Follow-me can be used again.** Deploy 2 is LIVE too (`b304609`, 2026-09-25 night): the OAK session restarts itself after 5 s without a fresh NN packet (10 s warm-up), with a cap of 3 restarts in 15 min, then a latched `color_stall_fault` (health) until a fresh packet or a service restart.
   ...
   - Charger-flag logging is LIVE (`6ef165b`): `bms.charger_connected`, `bms.load_connected`, `bms.bms_mode` in the per-tick log, and a WARNING `BMS charger flag: connected|disconnected ...` on each change. Test not done: the BMS Bluetooth went quiet at 20:55 (35 min on the charger at 100 %, 0 A; "Device ... was not found" since). A Daly BMS sleeps its Bluetooth without current; it should wake when current flows. Test: with the BMS awake, unplug the charger for 30 s and plug it back in, then read the journal.
   - I2C BUS 1 FROZE 2026-09-25 21:36:16 (kernel: `i2c_designware 1f00074000.i2c: i2c_dw_handle_tx_abort: SDA stuck at low`). A device held SDA low; the GPS co-processor (0x20) and the UPSPlus HAT (read by `upsplus-power.service`, the safe-shutdown watcher) both went dark for 17 h. The UPS safe shutdown on power loss was OFF for that time. The bus had no I2C errors in the 3.8 days before; no power dip, no restart, robot parked on the charger. Suspect (not proven): the GPS co-processor, the most frequent user of the bus, whose read failed first. The Pi 5 driver has no GPIO bus recovery, so only a power cycle cleared it (done 2026-09-26 15:06; GPS back, RTK float at 15:15).
   - Fix LIVE 2026-09-26 (`2759465`, Kevin chose software over rewiring): `i2c-bus-guard.service` (root, `bin/i2c_bus_guard.py`, doc `docs/i2c_bus_guard.md`). When the UPS status is stale > 30 s AND 10 joint samples show SDA low with SCL high, it unbinds the driver, clocks SCL (open-drain emulation, never drives high), sends a STOP, restores `a3 pu`, rebinds, and restarts `upsplus-power` (that daemon never reopens its bus handle). It self-heals a half-finished recovery on every cycle, warns whenever the UPS status is stale for any cause, and restarts the UPS daemon if its status stays stale > 120 s on a healthy bus (once per 15 min). Status: `/tmp/i2c_guard_status.json`. It was verified with a dry run on the real pins; the live unlock was NOT exercised (Kevin skipped it), so the first real use will be a real freeze. The guard does not restart on auto-deploy: after changing it, run `sudo bash bin/install_i2c_bus_guard.sh`. If wall-e starts while the bus is stuck, GPS stays disabled until the next wall-e restart.
...
5. **Phantom obstacles from repeated structures (gate, siding, gravel).** Fix built 2026-09-26 on `fm-speed` (`8a4afb2`, `04ba25d`): a depth-agreement speckle filter and a depth-aware person mask in the depth corridor. Replay-validated; field validation in motion is still open.
   - 2026-09-26 13:07 (`arm_20260926_130713.log`): Kevin stood in front of the iron gate. From 4.7 m to the stop, the corridor reported 0.56 m to 0.9 m (`corridor_near_px` 1,300 to 3,100, flickering around the support floor). Throttle 0.2 to 0.6 for 13 s: the approach was slow and jerky and started to slow early. 12:55: the same signature for 6 s next to the barn.
   - Evidence: 45 raw frames (`logs/depth_snapshots/*.npz` on the robot) from 11 setups that Kevin placed with the robot parked and disarmed. The gate phantoms are false stereo matches: sparse near pixels (median blob 2 px) at random depths from 0.35 m to 1.5 m, with nothing in front.
   - Filter: a near pixel (<= `slow_distance_m`) stays only when at least `corridor_speckle_min_neighbours` (16) near pixels in its `corridor_speckle_window_px` (13) square lie within one 100 mm depth bin of it. It runs only when the near pixels reach the support floor and stay under `corridor_speckle_max_near_px` (20,000). The density and support checks use the pixels before the filter; if too few remain to measure, the unfiltered reading stands (log `obstacle.corridor_speckle_fallback`). Log `obstacle.corridor_speckle_px` counts dropped pixels. About 7 ms per frame on the Pi 5 when it runs (the vision thread uses 11 of 66 ms today).
   - A blob-size filter (`8a4afb2`) was tried first and replaced: Grok and a stress test showed that woven wire fence looks like speckle at the pixel level; with 30 percent of the fence pixels removed, every blob variant lost the fence at about 1 m. Dropping pixels could also push valid density under `min_valid_pct` and publish "clear".
   - Replay of the production code over 150 frames (the 45 plus random and 8x8-block dropout to 70 percent): no real obstacle that the old code saw inside 1.5 m was lost, and no gate phantom was left. Old -> new: gate phantoms 0.70-0.81 m -> 2.1-2.2 m (ground edge); gate bars 1.00 -> 1.03 m; trash can 0.87 m (same); wire fence at 3 ft 0.79 -> 0.86 m (1.2 m at 70 percent density); wire fence at 5 ft 0.58-0.73 -> 1.64-1.71 m (its histogram peak is 1.6-1.8 m); siding in shade 1.73 m (same); sunlit siding at a taped 53 in (1.346 m; stereo median 1.341 m) 0.73 m (same, above the 20,000 px gate). The zero-loss, zero-phantom band is 12 to 22 neighbours at window 13.
   - Person mask: `person_mask_depth_margin_m` 0.5. The corridor used to zero the whole person box, which also hid an obstacle between the robot and the person inside that outline (Kevin asked why people are masked). Now only pixels at or beyond the person's range minus 0.5 m are masked. A person with an unknown range is not masked. An obstacle less than 0.5 m in front of the person is still masked (as before).
   - Still open: (a) a drive-by validation at the gate and along the barn; (b) a smooth, low-texture real obstacle (garage door, painted wall, dim light); (c) the sunlit siding reads about 0.6 m short (a 5 percent tail of false near pixels inside the wall's own blob sets the k-th smallest value): cautious, not a hazard. Rollback: `corridor_speckle_min_neighbours = 0`, `person_mask_depth_margin_m = 0.0`.
   - Tool: `POST /api/debug/depth_snapshot` on port 8080, local-only (live since `c284b28`). Run on the robot: `curl -s -X POST -H "Content-Type: application/json" -d '{"count": 5, "interval_s": 0.5}' http://127.0.0.1:8080/api/debug/depth_snapshot`. Cap 200 files. Colourized depth in the recorder cannot tell a false match from a hole (both dark blue): use raw frames.
6. **Steering gain.** Left at `pid_lateral_kp` 0.5 and cap 32 on purpose. The simulation shows margin. Since 2026-09-26 the follow-me forward cap is 128 (byte 254, the mixer limit), thus at top speed each steer correction is inside-wheel braking only and gives about half the yaw of the same correction at forward 110 or less. Optional experiment with Kevin present: `POST /api/follow_me/params {"pid_lateral_kp": 0.6}`. It needs no deploy and reverts in seconds.
