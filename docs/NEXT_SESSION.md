...
Updated 2026-09-26 (afternoon): follow-me top speed 1.61 m/s, follow range 8 m and a 4 ft stop are LIVE (`c3c995a`, deployed 13:45). Phantom obstacles in front of an iron gate and lap siding: section 4, item 5.
...
5. **Phantom obstacles from repeated structures (gate, siding, gravel).** Open, highest follow-me priority after 2026-09-26.
   - 2026-09-26 13:07 (`arm_20260926_130713.log`): Kevin stood in front of the iron gate. From 4.7 m to the stop, the corridor reported 0.56 m to 0.9 m (`corridor_near_px` 1,300 to 3,100, flickering around the support floor of about 2,000 px). Throttle 0.2 to 0.6 for 13 s: the approach was slow and jerky and started to slow early.
   - 2026-09-26 12:55 (`arm_20260926_125554.log`): the same signature for 6 s in front of the barn lap siding (0.6 m reported, robot crept at 0.2 m/s to 0.4 m/s).
   - The reported range did not decrease while the robot drove toward it, and the corridor band (rows 25 to 75 percent) cannot see the ground nearer than about 2.3 m. Thus these are false stereo matches, not the ground. Suspect: the repeated vertical bars and horizontal boards. The 2026-09-20 15:49 gravel phantom (0.5 m to 0.9 m for 3 s, 9 to 11 percent valid pixels) is probably the same class.
   - CAUTION: the recorder's colourized depth (`logs/oak/<session>/session.mcap`, topic `/oak/depth_colorized`) shows invalid pixels and near pixels in the same dark blue. It cannot tell a false match from a hole. Use raw frames.
   - Tool (`508b0ae`, deploy pending): `POST /api/debug/depth_snapshot` on port 8080, local-only. Run on the robot: `curl -s -X POST -H "Content-Type: application/json" -d '{"count": 5, "interval_s": 0.5}' http://127.0.0.1:8080/api/debug/depth_snapshot`. Files: `logs/depth_snapshots/*.npz` (raw uint16 depth, RGB JPEG, person boxes, intrinsics, corridor stats).
   - Next: park the robot (disarmed) facing the gate at about 3 m, 2 m and 1.5 m, and facing the siding. Capture at each position. Rebuild the corridor offline and compare candidate filters (per-block near fraction of valid pixels; device stereo confidence or speckle filter). A filter must not lose a real post or a real wall: test those too. Grok safety review before any push.
