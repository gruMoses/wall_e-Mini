...
class WaypointNavController:
    ...
    def compute(
        ...
        ``current_heading_deg`` must already be in the true-north frame;
        the controller is responsible for applying any GPS-derived offset
        (see ``GpsHeadingAligner``) before calling in.
