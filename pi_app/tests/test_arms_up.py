...
class TestArmsUpPure(unittest.TestCase):
    ...
    def test_low_visibility_elbow_is_ignored(self):
        self.assertTrue(arms_up(_pose(l_el_y=0.80, l_el_vis=0.1), CFG))
...
class TestArmsUpDetector(unittest.TestCase):
    ...
    def test_hold_requires_new_sample_timestamps(self):
        ...
        # Same ts, later wall clock: must not advance the streak.
        st = det.update(up, now=1.3)
        self.assertFalse(st["active"])
        self.assertEqual(st["streak_s"], 0.0)
        self.assertAlmostEqual(st["last_sample_age_s"], 0.3)
