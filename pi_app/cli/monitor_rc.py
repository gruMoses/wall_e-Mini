...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Monitor Arduino RC channels (CSV: ch1,ch2,ch3,ch5)")
    ...
    try:
        while should_run:
            ...
            print(
                f"{status} ch1={state.ch1_us:4d}  ch2={state.ch2_us:4d}  ch3={state.ch3_us:4d}  ch5={state.ch5_us:4d}  age={age_s:4.1f}s",
                end="\r",
                flush=True,
            )
            time.sleep(period_s)
